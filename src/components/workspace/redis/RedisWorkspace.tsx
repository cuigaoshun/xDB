...
export function RedisWorkspace({tabId, name, connectionId, db = 0, savedResult}: {
    tabId: string;
    name: string;
    connectionId: number;
    db?: number;
    ...
    const fetchListValues = useCallback(async (start = 0, end = 99, forcedType?: string) => {
        ...
        try {
            ...
            if (start === 0) {
                ...
                setAllValues((prev) => [...prev, ...result.output]);
            }
    ...
    useEffect(() => {
        ...
        if (!selectedKey || !isKeyChanged) return;
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            (entries) => {
                ...
                if (valueHasMoreRef.current && !valueLoadingRef.current &&
                    selectedKeyRef.current && currentKeyItem) {
                    const type = currentKeyItem.type;
                    if (type === "hash" || type === "set" || type === "zset") {
                        fetchComplexValues(false);
                    } else if (type === "list") {
                        fetchListValues(allValuesRef.current.length, allValuesRef.current.length + 99);
                    }
                }
            },
            {threshold: 0, rootMargin: '100px'}
                            ...
                            <div className="flex items-center gap-1">
                                <div className="relative flex-1">
                                    ...
                                    {isInputFocused && suggestedHistory.length > 0 && (
                                        <div
                                            className="absolute top-full left-0 right-0 mt-1 bg-popover text-popover-foreground border rounded-md shadow-md z-50 py-1 max-h-[300px] overflow-auto">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuItem
                                                onClick={() => setRedisViewMode(connectionId, db, 'list')}>
                                                <List className="mr-2 h-4 w-4"/>
                                                {t('redis.listView')}
                                            </DropdownMenuItem>
                                            <DropdownMenuItem
                                                onClick={() => setRedisViewMode(connectionId, db, 'tree')}>
                                            ...
                                            <div className="px-2 py-1.5">
                                                <label
                                                    className="text-xs text-muted-foreground">{t('redis.delimiter')}</label>
                                                <Input
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        className="flex flex-col items-end text-[10px] text-muted-foreground shrink-0 whitespace-nowrap">
                    ...
                    <ResizablePanel defaultSize={40}>
                        {selectedKey ? (
                            <div className="h-full flex flex-col">
                                ...
                                {/* Metadata Bar */}
                                <div
                                    className="px-4 py-2 border-b bg-muted/10 flex gap-6 text-xs text-muted-foreground">
                                    <div className="flex gap-1">
