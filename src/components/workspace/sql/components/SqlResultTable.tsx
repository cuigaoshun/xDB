...
export const sortNumber = (valA: any, valB: any, order: "asc" | "desc") => {
...
export const sortDate = (valA: any, valB: any, order: "asc" | "desc") => {
    const timeA = new Date(valA).getTime();
    const timeB = new Date(valB).getTime();
    
    if (isNaN(timeA) && isNaN(timeB)) return 0;
    if (isNaN(timeA)) return order === "asc" ? -1 : 1;
    if (isNaN(timeB)) return order === "asc" ? 1 : -1;
    
    return order === "asc" ? timeA - timeB : timeB - timeA;
};

export const sortString = (valA: any, valB: any, order: "asc" | "desc") => {
...
export function SqlResultTable({
    ...
    const [inlineSort, setInlineSort] = useState<{ column: string; order: "asc" | "desc" } | null>(null);

    const sortedRowEntries = useMemo(() => {
        if (!inlineSort) return filteredRowEntries;
        
        const columnDef = result.columns.find((c) => c.name === inlineSort.column);
        const typeName = (columnDef?.type_name || "").toUpperCase();
        
        const isNumber = typeName.includes("INT") || typeName.includes("FLOAT") || typeName.includes("DOUBLE") || typeName.includes("DECIMAL") || typeName.includes("BOOL") || typeName.includes("NUMERIC") || typeName.includes("REAL");
        const isDate = typeName.includes("DATE") || typeName.includes("TIME") || typeName.includes("TIMESTAMP") || typeName.includes("YEAR");

        return [...filteredRowEntries].sort((a, b) => {
            const valA = a.row[inlineSort.column];
            const valB = b.row[inlineSort.column];
            
            if (valA === null && valB === null) return 0;
            if (valA === null) return inlineSort.order === "asc" ? -1 : 1;
            if (valB === null) return inlineSort.order === "asc" ? 1 : -1;
            
            if (isNumber) {
                return sortNumber(valA, valB, inlineSort.order);
            } else if (isDate) {
                return sortDate(valA, valB, inlineSort.order);
            } else {
                return sortString(valA, valB, inlineSort.order);
            }
        });
    }, [filteredRowEntries, inlineSort, result.columns]);
    ...
    const filteredRowIndices = useMemo(
        () => sortedRowEntries.map((entry) => entry.originalIndex),
        [sortedRowEntries],
    );
                ...
                >
                    <Table className="table-fixed" containerClassName="overflow-visible">
                                    ...
                                    <TableHead className="w-[50px] min-w-[50px] p-0">
                                        <div className="flex items-center justify-center h-full w-full">
                                            <input
                                                ...
                                                checked={sortedRowEntries.length > 0 && sortedRowEntries.every((entry) => selectedRowIndexSet.has(entry.originalIndex))}
                                                onChange={(event) => {
                                                ...
                                                <DropdownMenuTrigger asChild>
                                                    <Button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            (inlineFilters[column.name] || inlineSort?.column === column.name) && "text-blue-600 opacity-100",
                                                        )}
                                                    >
                                                    ...
                                                    <div className="p-2 flex items-center gap-1">
                                                        <div className="relative flex-1 min-w-0">
                                                            <Input
                                                                ...
                                                                value={inlineFilters[column.name] || ""}
                                                                onChange={(e) => onInlineFilterChange(column.name, e.target.value)}
                                                                onKeyDown={(e) => e.stopPropagation()}
                                                                onClick={(e) => e.stopPropagation()}
                                                                className="h-8 text-xs focus-visible:ring-1 focus-visible:ring-offset-0 w-full pr-8"
                                                            />
                                                            {inlineFilters[column.name] && (
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
                                                                    className="absolute right-0 top-0 h-8 w-8 p-0 hover:bg-transparent text-muted-foreground hover:text-foreground"
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                                        onInlineFilterChange(column.name, "");
                                                                    }}
                                                                >
                                                                    <X className="h-3 w-3" />
                                                                </Button>
                                                        ...
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            className={cn("h-8 w-8 p-0 flex-shrink-0 transition-colors", inlineSort?.column === column.name && "bg-accent text-accent-foreground")}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                if (inlineSort?.column !== column.name) {
                                                                    setInlineSort({ column: column.name, order: "asc" });
                                                                } else if (inlineSort.order === "asc") {
                                                                    setInlineSort({ column: column.name, order: "desc" });
                                                                } else {
                                                                    setInlineSort(null);
                                                                }
                                                            }}
                                                            title={inlineSort?.column !== column.name ? t("common.orderBy", "排序") : inlineSort.order === "asc" ? t("common.ascending", "升序") : t("common.descending", "降序")}
                                                        >
                                                            {inlineSort?.column !== column.name ? (
                                                                <ArrowUpDown className="h-4 w-4 text-muted-foreground" />
                                                            ) : inlineSort.order === "asc" ? (
                        ...
                        <TableBody>
                                        ...
                                        <TableCell key={`${column.name}-${colIdx}`} className="whitespace-nowrap">
                                            {editingCell?.rowIdx === rowIdx && editingCell?.colName === column.name && editingCell?.isNewRow ? (
                                                <div className="relative w-full">
                                                    ...
                                                    />
                                                    <div className="absolute right-2 top-0 h-full flex items-center gap-0.5 pr-2">
                            ...
                            {sortedRowEntries.length === 0 && newRows.length === 0 && (
                                <TableRow>
                                    <TableCell
