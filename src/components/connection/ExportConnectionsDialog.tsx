...
export function ExportConnectionsDialog({ open, onOpenChange }: ExportConnectionsDialogProps) {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            // First, collect group IDs from selected connections (before deleting group_id)
            ...
            // Export groups
            const groupsToExport = connectionGroups
                ...
                .map(g => {
                    ...
                    // @ts-ignore
                    delete exportGroup.created_at;
                    return exportGroup;
                });

            // Then process connections
            const connectionsToExport = selectedConnections.map(c => {
                const exportConn = { ...c };
                ...
                // Replace group_id with group_name for portability
                if (exportConn.group_id) {
                    const group = connectionGroups.find(g => g.id === exportConn.group_id);
                    if (group) {
                        // @ts-ignore
                        exportConn.group_name = group.name;
                    }
                }
                // @ts-ignore
                delete exportConn.group_id;
