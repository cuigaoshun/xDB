...
interface AppState {
  ...
  removeConnectionGroup: (id: number) => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setExpandedConnectionId: (id) => {
    if (id !== null) {
      localStorage.setItem('xdb-last-expanded-connection', String(id));
    } else {
      localStorage.removeItem('xdb-last-expanded-connection');
    }
    set({ expandedConnectionId: id, activeView: 'connections' });
  },
  ...
  addConnectionGroup: (group) => set((state) => ({
    connectionGroups: [group, ...state.connectionGroups]
  })),

  updateConnectionGroup: (group) => set((state) => ({
    connectionGroups: state.connectionGroups.map(g => g.id === group.id ? group : g)
  })),

  removeConnectionGroup: (id) => set((state) => ({
